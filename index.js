    ...
    (error, response, body) => {
      ...
      if (response.statusCode === 200) {
        if (response.body.status === 1) {
          ...
          const data = response.body.data.yy_date.filter((item) =>
            dataArr.includes(item.t_date)
          );
          ...
          shouldEmail = response.body.data.yy_date
            .filter((item) =>
              ["2024-05-03", "2024-05-04"].includes(item.t_date)
            )
            .some((t) => t.tp_last_stock_sum > 0);

          // console.log()

          // return
          // if( response.body.data.yy_date.filter(item => ['2024-05-03', '2024-05-04'].includes(item.t_date)).some(t => t))
        } else {
          subject = "统计失败";
        ...
        nodemailer.createTestAccount((err, account) => {
          ...
          let mailOptions = {
            ...
            html:
              replyStr +
              "<br>" +
              '网站地址：<a  href="https://pw.hbww.org.cn/personal/index">https://pw.hbww.org.cn/personal/index </a> ',
          };
          // 发送
